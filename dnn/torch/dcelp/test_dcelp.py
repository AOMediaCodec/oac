...
parser.add_argument('quant', type=int, help='quantizer')
parser.add_argument('features', type=str, help='path to feature file in .f32 format')
parser.add_argument('audio', type=str, help='path to feature file in .f32 format')
...
#features[:,:,-16:] = 10*(features[:,:,-16:]-(np.arange(1, 17)[None,None,:])/18)
...
audio = np.reshape(np.memmap(audio_file, dtype='int16', mode='r'), (1, -1))
...
if __name__ == '__main__':
    ...
    #features[:,1:-1:2,:] = .5*(features[:,:-2:2,:]+features[:,2::2,:])
    audio = torch.tensor(audio).to(device)
    np.savetxt('scale.txt', model.fsq.scale.detach().cpu().numpy())

    audio = dcelp.process_audio(audio)
    audio = audio[:,640:]
    q = torch.tensor([args.quant])
